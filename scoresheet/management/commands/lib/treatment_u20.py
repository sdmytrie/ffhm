        ...
        for gender in super().genders:
            for kind in super().kinds:
                _all_records = Record.objects.filter(
                    agecategory="U20", gender=gender, kind=kind
                )
                for record in _all_records:
                    _buffer_weightcategory = record.weightcategory
                    if (
                        record.weightcategory == "110"
                        or record.weightcategory == ">110"
                    ) and gender.name == "male":
                        _buffer_weightcategory = ">94"
                    elif (
                        record.weightcategory == "86" or record.weightcategory == ">86"
                    ) and gender.name == "female":
                        _buffer_weightcategory = ">77"
    ...
    def set_record_from_last_agecategory(self):
        for gender in super().genders:
            _current_agecategory = Agecategory.objects.get(
                name="SENIOR", season=super().current_season, gender__name=gender.name
            )
            _last_agecategory = Agecategory.objects.get(
                name="U20", season=super().last_season, gender__name=gender.name
            )
            ...
            for event in _events:
                ...
                # _event = _ffhm_agecategory.get_last_agecategory(event)
                if _age - 1 > _last_agecategory.agemax:
                    continue
                for kind in super().kinds:
                    # _ffhm_record = FfhmRecord(gender, "U20", event.weightcategory, kind)
                    _ffhm_record = FfhmRecord(gender.name, "U20", None, kind)
