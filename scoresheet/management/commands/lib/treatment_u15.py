        ...
        for gender in super().genders:
            for kind in super().kinds:
                _all_records = Record.objects.filter(
                    agecategory="U15", gender=gender, kind=kind
                )
    ...
    def set_record_from_last_agecategory(self):
        for gender in super().genders:
            _current_agecategory = Agecategory.objects.get(
                name="U17", season=super().current_season, gender__name=gender.name
            )
            _last_agecategory = Agecategory.objects.get(
                name="U15", season=super().last_season, gender__name=gender.name
            )
            ...
            for event in _events:
                ...
                if _age - 1 > _last_agecategory.agemax:
                    continue
                for kind in super().kinds:
                    _ffhm_record = FfhmRecord(gender.name, "U15", None, kind)
                    if (
                        event.weightcategory.weight == "77"
                        or event.weightcategory.weight == ">77"
                    ) and gender.name == "female":
                        _ffhm_record.weight = ">69"
                    elif (
                        event.weightcategory.weight == "94"
                        or event.weightcategory.weight == ">94"
                    ) and gender.name == "male":
                        _ffhm_record.weight = ">88"
                    else:
