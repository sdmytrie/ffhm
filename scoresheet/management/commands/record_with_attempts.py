...
class Command(BaseCommand):
    def get_weightcategory(self, event):
        ...
        current_agecategory = (
            Agecategory.objects.prefetch_related("gender")
    ...
    def get_all_attempt_by_name_and_gender(
        self, name, gender, agecategory, weightcategory
    ):
        attempt_all = list(
        ...
        manager_records = ManageRecords()
        for attempt in attempt_all:
            event = manager_records.get_last_agecategory(attempt.event)
            if not event:
                continue
            for new_attempt in event.attempt_set.all():
                if not new_attempt.name == name:
                    continue
                attempt_all.append(new_attempt)
        # attempt_all = sorted(attempt_all, key=lambda x: x.value)
        # # attempt_all = sorted(attempt_all, key=lambda x: x.updated_at)
        # max = 0
        # good = None
        # for attempt in attempt_all:
        #     if attempt.value > max:
        #         max = attempt.value
        #         good = attempt
        #     elif attempt.value == max:
        #         if attempt.updated_at < good.updated_at:
        #             good = attempt
        for buffer_attempt in attempt_all:
            buffer_weightcategory = self.get_weightcategory(buffer_attempt.event)
            if not buffer_weightcategory == weightcategory.weight:
                continue
            buffer_attempt.event.weightcategory = buffer_weightcategory
        return attempt_all
                ...
                for age in age_categories:
                    ...
                    for weightcategory in current_agecategory.weightcategory_set.all():
                        ...
                        if attempt_type == "TOTAL":
                            ...
                            #     validate=1,
                            # ).order_by("-value", "updated_at")
                            attempt_all = self.get_all_attempt_by_name_and_gender(
                                attempt_type,
                                gender,
                                current_agecategory,
                                weightcategory,
                            ...
                            # for buffer_attempt in attempt_all:
                            #     buffer_weightcategory = self.get_weightcategory(
                            #         buffer_attempt.event
                            #     )
                            #     if not buffer_weightcategory == weightcategory.weight:
                            #         continue
                            #     buffer_attempt.event.weightcategory = (
                            #         self.get_weightcategory(buffer_attempt.event)
                            #     )
                            # attempt_all = list(
                            #     filter(
                            #         lambda x: x.event.weightcategory.weight
                            ...
                            # attempt_all = list(
                            #     filter(
                            #         lambda x: x.event.agecategory.name
                            #         == current_agecategory.name,
                            #         attempt_all,
                            #     )
                            # )
